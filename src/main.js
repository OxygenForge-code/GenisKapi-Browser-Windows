...
async function installChromeWebStoreExtension(id,storeUrl=''){
  ...
  const crx=await downloadCrx(id); uiProgress({id,stage:'downloaded',progress:30,text:'Paket alındı'});
  const zip=parseCrx(crx);
  const baseDir=path.join(app.getPath('userData'),'extensions'); const finalDir=path.join(baseDir,id); const tempDir=path.join(baseDir,`${id}.installing-${Date.now()}`);
  fs.mkdirSync(baseDir,{recursive:true}); fs.mkdirSync(tempDir,{recursive:true});
  try{
    new AdmZip(zip).extractAllTo(tempDir,true); const manifest=readManifest(tempDir); uiProgress({id,stage:'unpack',progress:60,text:'Uzantı hazırlanıyor…'});
    ...
    const record={id:loaded.id||id,name:manifest.name,version:manifest.version,manifestVersion:manifest.manifest_version,path:finalDir,source:'Chrome Web Store',storeUrl:storeUrl||`https://chromewebstore.google.com/detail/${id}`};
    extensions.set(record.id,record); writeInstalledExtensions(); win?.webContents.send('extension-installed',record); uiProgress({id:record.id,stage:'done',progress:100,text:`${record.name} kuruldu`}); return{ok:true,extension:record};
  }catch(error){try{fs.rmSync(tempDir,{recursive:true,force:true});}catch{} try{fs.rmSync(finalDir,{recursive:true,force:true});}catch{} throw error;}
}
...
function injectCwsBridge(contents){
  contents.on('did-finish-load',async()=>{
    ...
    const bridge=`(()=>{
      ...
      const hideWarning=()=>document.querySelectorAll('body *').forEach(el=>{if(!(el instanceof HTMLElement)||el.children.length>4)return;const t=(el.innerText||'').trim();if(t&&t.length<180&&/(chrome|google chrome).*(gerek|gerekiyor|required|needed)/i.test(t))el.style.display='none'});
      const markButtons=()=>document.querySelectorAll('button,[role="button"],a').forEach(el=>{const t=(el.innerText||el.getAttribute('aria-label')||'').trim();if(/^add to chrome$|^chrome'a ekle$/i.test(t)){el.dataset.gkInstall='1';if(el instanceof HTMLElement)el.innerText="GenişKapı'ya ekle"}});
      document.addEventListener('click',event=>{const el=event.target instanceof Element?event.target.closest('[data-gk-install],button,[role="button"],a'):null;if(!el)return;const t=(el.innerText||'').trim();if(el.dataset.gkInstall!=='1'&&!/chrome'a ekle|add to chrome/i.test(t))return;const id=getId();if(!id)return;event.preventDefault();event.stopPropagation();event.stopImmediatePropagation();location.href='geniskapi://install/'+id+'?source='+encodeURIComponent(location.href)},true);
      new MutationObserver(()=>{hideWarning();markButtons()}).observe(document.documentElement,{childList:true,subtree:true});hideWarning();markButtons();
    })();`;
...
function createWindow(){
  ...
  view.webContents.on('will-navigate',async(event,url)=>{if(!url.startsWith('geniskapi://install/'))return;event.preventDefault();try{const u=new URL(url);const id=u.pathname.replace(/^\//,'').split('/').pop();const source=u.searchParams.get('source')||'';const result=await installChromeWebStoreExtension(id,source);win.webContents.send('extension-install-result',result)}catch(error){win.webContents.send('extension-install-error',{error:error.message});uiProgress({stage:'error',progress:0,text:`Kurulum başarısız: ${error.message}`})}});
  view.webContents.loadURL('https://www.google.com/'); win.loadFile(path.join(__dirname,'ui.html'));
  win.webContents.on('did-finish-load',()=>{try{const css=fs.readFileSync(path.join(__dirname,'animations.css'),'utf8');win.webContents.insertCSS(css).catch(()=>{})}catch{}}); win.on('resize',resize);
...
ipcMain.handle('back',()=>view.webContents.goBack()); ipcMain.handle('forward',()=>view.webContents.goForward()); ipcMain.handle('reload',()=>view.webContents.reload()); ipcMain.handle('home',()=>view.webContents.loadURL('https://www.google.com/'));
...
ipcMain.handle('extensions-list',()=>[...extensions.values()]); ipcMain.handle('extensions-remove',async(_,id)=>{if(!extensions.has(id))return false;try{await session.defaultSession.removeExtension(id)}catch{}extensions.delete(id);writeInstalledExtensions();return true});
